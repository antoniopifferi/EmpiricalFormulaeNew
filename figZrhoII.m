...
M=500;
rho=[0.4:1000];
filename='figZrho.xlsx';

i=0;
for s0=[10000]
    for mua=[0,1e-15,1e-10,1E-6,5e-5,7.5e-5,1e-4,2.5e-4,5e-4,7.5e-4,1e-3,2.5e-3,5e-3,7.5e-3,1e-2,2.5e-2,5e-2,7.5e-2,1e-1,2.5e-1,5e-1,7.5e-1,1]
        i=i+1;
        for musp=[1]
            for nout=1.
                for nin=[1.]
                    
                    Zrho=funZrho(s0,rho,mua,musp,nin,nout,M);
                    MUA(i)=mua;
                    ZRHO(:,i)=Zrho';
...
if exist(filename,'file')
    delete(filename);
    xlswrite(filename,rho',1,'A3');
    xlswrite(filename,MUA,1,'B1');
    xlswrite(filename,ZRHO,1,'B3');
else
    xlswrite(filename,rho',1,'A3');
    xlswrite(filename,MUA,1,'B1');
    xlswrite(filename,ZRHO,1,'B3');
...
ylabel('$\langle z_{max} | \rho\rangle$  (mm)','interpreter','latex')
grid
